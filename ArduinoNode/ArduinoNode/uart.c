...
void configure_uart(void) {
  ...
  */
  ...
  // Disable the Parallel IO (PIO) of the URXD and UTXD pins so that the
  // peripheral controller can use them
  PIOA->PIO_PDR = PIO_PA8A_URXD | PIO_PA9A_UTXD;

  // Read current peripheral AB select register and set the UTXD and URXD pins
  // to 0 (UART is connected as peripheral A)
  ul_sr = PIOA->PIO_ABSR;
...
int uart_putchar(const uint8_t c) {
  ...
  while (!((UART->UART_SR) & UART_SR_TXEMPTY))
    ; // Wait for the character to be sent, can implement ring buffer to remove
      // the wait
  return 0;
}
